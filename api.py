...
from fastapi import Body, FastAPI, HTTPException, Depends  # For the API
...
async def get_current_user(credentials: HTTPAuthorizationCredentials = Depends(security)):
    ...
    try:
        ...
        return {"user": db_service.get_user_by_id(user_id)}
...
async def get_all_posts():
    ...
    return {"posts": db_service.get_all_posts()}
...
async def get_posts_with_tag(tag: str):
    ...
    return {"posts": db_service.get_posts_with_tag(tag)}
...
async def get_tags_of_post(post_id: int):
    ...
    return {"tags": db_service.get_tags_of_post(post_id)}
...
async def get_user_by_id(user_id: int, current_user: dict = Depends(get_current_user_id)):
    ...
    current_user = db_service.get_user_by_id(current_user_id)
...
@app.get("/user/name/{username}/role/")  # TODO: Use id instead of name
async def get_role_of_user_by_name(username: str):
    ...
    role = db_service.get_role_of_user_by_name(username)
...
async def get_chat_by_id(chat_id: int):
    ...
    """
    return {"chat": db_service.get_chat_by_id(chat_id)}
...
async def get_messages_of_chat(chat_id: int):
    ...
    return {"messages": db_service.get_messages_of_chat(chat_id)}
...
async def get_chats_of_user(current_user: dict = Depends(get_current_user_id)):
    ...
    return {"chats": db_service.get_chats_of_user(current_user_id)}
...
async def create_user(user: SignupData):
    ...
    db_service.create_user(user.username,
                           user.email,
                           user.password)
...
async def create_comment(post_id: int, comment: Comment, current_user: dict = Depends(get_current_user_id)):
    ...
    if not (db_service.get_post_by_id(post_id) and current_user_id):
...
async def update_post(post_id: int, new_title: Annotated[str, Body()], new_content: Annotated[str, Body()]):
    ...
    Args:
        post_id: The ID of the post (integer).
        new_title: The new title of the post (string).
...
async def update_comment(post_id: int, comment_id: int, new_content: Annotated[str, Body()]):
    """
